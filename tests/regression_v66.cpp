...
int main()
{
    ...
    for (int applied = 25; applied <= 100; ++applied)
    for (int staged : {25, 99, 100}) test_sharpness_interaction(available, applied, staged);
    std::puts("Sharpness: 456 click cases passed; applied 100 disabled, 25-99 enabled when available; staged scale ignored and disabled value preserved; full Cost Scaler controls present.");
