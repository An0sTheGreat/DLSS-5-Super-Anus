...
std::array<std::atomic_int, 3> g_control_keys = {0x75, 0x76, 0x74};
...
constexpr const char *kBindingNames[] = {"NRToggleKey", "PresetCycleKey", "NRScreenshotKey"};
constexpr const char *kBindingLabels[] = {"Toggle Neural Rendering", "Cycle Preset 1 -> 2 -> 3 -> 1", "Capture NR ON/OFF pair"};
void request_screenshot();

std::array<int, 3> control_keys()
{
...
std::array<bool, 3> poll_control_keys(Runtime *runtime)
{
    DWORD foreground_pid = 0;
    GetWindowThreadProcessId(GetForegroundWindow(), &foreground_pid);
    ...
    if (runtime && focused && action >= 0 && action < 3) {
