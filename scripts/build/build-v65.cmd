...
cl /nologo /c /std:c++20 /MD /EHs-c- /O2 /Oi /GS- /GR- /guard:cf- /Zl /Brepro /W4 /WX ^
  ...
  /Fo"%ROOT%build\neural_resolution_v65.obj" "%ROOT%src\neural_resolution_addon.cpp" || exit /b 1
ml64 /nologo /c /Fo"%ROOT%build\embedded_bridges_v65.obj" "%ROOT%src\embedded_bridges.asm" || exit /b 1
link /nologo /dll /nodefaultlib /entry:combined_entry /dynamicbase /incremental:no /Brepro /opt:ref /opt:icf ^
  /map:"%ROOT%build\neural_resolution_v65.map" /out:"%ROOT%build\neural_resolution_v65_embedded.dll" ^
  "%ROOT%build\neural_resolution_v65.obj" "%ROOT%build\embedded_bridges_v65.obj" ^
  kernel32.lib Psapi.lib User32.lib ucrt.lib || exit /b 1
python "%ROOT%tools\patch_v6_addon.py" --base "%ROOT%updated-official-renodx-dlss.addon64" ^
  --embedded "%ROOT%build\neural_resolution_v65_embedded.dll" --map "%ROOT%build\neural_resolution_v65.map" ^
  --section-name .nr-v65 --output "%ROOT%build\v65-candidate.addon64" || exit /b 1
python "%ROOT%tools\validate_v6_addon.py" --base "%ROOT%updated-official-renodx-dlss.addon64" ^
  --version V6.5 --addon "%ROOT%build\v65-candidate.addon64" || exit /b 1
endlocal
