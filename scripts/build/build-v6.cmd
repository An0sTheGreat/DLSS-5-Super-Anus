...
cl /nologo /c /std:c++20 /EHs-c- /O2 /Oi /GS- /GR- /guard:cf- /Zl /Brepro /W4 ^
  ...
  /Fo"%ROOT%build\neural_resolution_v6.obj" "%ROOT%src\neural_resolution_addon.cpp" || exit /b 1
ml64 /nologo /c /Fo"%ROOT%build\embedded_bridges.obj" "%ROOT%src\embedded_bridges.asm" || exit /b 1

link /nologo /dll /nodefaultlib /entry:combined_entry /dynamicbase /incremental:no /Brepro /opt:ref /opt:icf ^
  /map:"%ROOT%build\neural_resolution_v6.map" ^
  /out:"%ROOT%build\neural_resolution_v6_embedded.dll" ^
  "%ROOT%build\neural_resolution_v6.obj" "%ROOT%build\embedded_bridges.obj" ^
...
python "%ROOT%tools\patch_v6_addon.py" ^
  ...
  --embedded "%ROOT%build\neural_resolution_v6_embedded.dll" ^
  --map "%ROOT%build\neural_resolution_v6.map" ^
  --output "%ROOT%release\renodx-dlss5Super-unified-neural-resolution-v6.addon64" || exit /b 1
python "%ROOT%tools\validate_v6_addon.py" ^
  --base "%ROOT%updated-official-renodx-dlss.addon64" ^
  --addon "%ROOT%release\renodx-dlss5Super-unified-neural-resolution-v6.addon64" || exit /b 1
endlocal
