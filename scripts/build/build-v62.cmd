...
cl /nologo /c /std:c++20 /EHs-c- /O2 /Oi /GS- /GR- /guard:cf- /Zl /Brepro /W4 /WX ^
  ...
  /Fo"%ROOT%build\neural_resolution_v62.obj" "%ROOT%src\neural_resolution_addon.cpp" || exit /b 1
ml64 /nologo /c /Fo"%ROOT%build\embedded_bridges_v62.obj" "%ROOT%src\embedded_bridges.asm" || exit /b 1

link /nologo /dll /nodefaultlib /entry:combined_entry /dynamicbase /incremental:no /Brepro /opt:ref /opt:icf ^
  /map:"%ROOT%build\neural_resolution_v62.map" ^
  /out:"%ROOT%build\neural_resolution_v62_embedded.dll" ^
  "%ROOT%build\neural_resolution_v62.obj" "%ROOT%build\embedded_bridges_v62.obj" ^
